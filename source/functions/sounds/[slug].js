const JSON_URL = 'https://raw.githubusercontent.com/imnotkoolkid/KCH/refs/heads/main/data/sounds.json';
const SITE_URL = 'https://kirkacommunityhub.pages.dev';
const FALLBACK_IMG = `${SITE_URL}/assets/icon.png`;
const FALLBACK_TITLE = 'Kirka Community Hub - Sounds';
const FALLBACK_DESC = 'Discover and download custom sounds for kirka.io';
...
function buildOgHtml({ title, owner, description, image, slug }) {
  ...
  const ogUrl = escapeHtml(`${SITE_URL}/sounds/${slug}`);
  const canonical = escapeHtml(`${SITE_URL}/assets.html#sounds#${encodeURIComponent(slug)}`);
...
<head>
  ...
  <script>
    window.location.replace(${JSON.stringify(`${SITE_URL}/assets.html#sounds#${slug}`)});
  </script>
</head>
<body>
...
export async function onRequestGet(context) {
  ...
  if (!isBotRequest(request)) {
    return Response.redirect(`${SITE_URL}/assets.html#sounds#${encodeURIComponent(slug)}`, 302);
  }
  ...
  try {
    ...
    const items = (await res.json()).sounds || [];
    const targetName = slugToName(slug);
    const item = items.find((i) => (i.name || '').trim().toLowerCase() === targetName);

    if (!item) return Response.redirect(`${SITE_URL}/assets.html#sounds`, 302);

    return new Response(buildOgHtml({ title: item.name, owner: item.owner || 'Unknown', description: '', image: '', slug }), {
      status: 200, headers: { 'Content-Type': 'text/html;charset=UTF-8', 'Cache-Control': 'public, max-age=300, s-maxage=300' }
    });
  } catch (err) {
    return Response.redirect(`${SITE_URL}/assets.html#sounds`, 302);
  }
}
