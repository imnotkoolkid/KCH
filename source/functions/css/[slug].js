...
// Same slug logic your frontend uses
const slugToTitle = (s) =>
...
<head>
  ...
  <!-- Immediate JS redirect for any browser that somehow lands here -->
  <script>
...
export async function onRequestGet(context) {
  ...
  // ── Human visitor → redirect immediately, no JSON fetch needed ──
  ...
  // ── Bot → fetch JSON, find item, return OG HTML ──
  try {
    const res = await fetch(CSS_JSON_URL, {
      headers: { 'User-Agent': 'KCH-OG-Worker/1.0' },
      // Cache at the edge for 5 minutes so bots don't hammer GitHub
    ...
    if (!item) {
      // Unknown slug → redirect bots to the main CSS page
    ...
    return new Response(html, {
      ...
      headers: {
        ...
        // Cache this rendered HTML at the edge for 5 minutes
        'Cache-Control': 'public, max-age=300, s-maxage=300',
      },
    });
  } catch (err) {
    // On any error, just redirect rather than showing a broken page
