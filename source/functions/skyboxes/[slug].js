...
function buildOgHtml({ title, owner, description, image, slug }) {
  ...
  const ogUrl = escapeHtml(`${SITE_URL}/skyboxes/${slug}`);
  const canonical = escapeHtml(`${SITE_URL}/assets.html#skyboxes#${encodeURIComponent(slug)}`);
...
<head>
  ...
  <script>
    window.location.replace(${JSON.stringify(`${SITE_URL}/assets.html#skyboxes#${slug}`)});
  </script>
</head>
<body>
...
export async function onRequestGet(context) {
  ...
  if (!isBotRequest(request)) {
    return Response.redirect(`${SITE_URL}/assets.html#skyboxes#${encodeURIComponent(slug)}`, 302);
  }
  ...
  try {
    ...
    const items = (await res.json()).skyboxes || [];
    const targetName = slugToName(slug);
    const item = items.find((i) => (i.name || '').trim().toLowerCase() === targetName);

    if (!item) return Response.redirect(`${SITE_URL}/assets.html#skyboxes`, 302);

    const image = item.isPack ? (item.images?.find(x => x.name === 'front')?.url || item.images?.[0]?.url || '') : (item.url || '');
    ...
    return Response.redirect(`${SITE_URL}/assets.html#skyboxes`, 302);
  }
}
