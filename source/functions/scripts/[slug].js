const JSON_URL = 'https://raw.githubusercontent.com/imnotkoolkid/KCH/refs/heads/main/data/script.json';
const SITE_URL = 'https://kirkacommunityhub.pages.dev';
const FALLBACK_IMG = `${SITE_URL}/assets/icon.png`;
const FALLBACK_TITLE = 'Kirka Community Hub - Scripts';
const FALLBACK_DESC = 'Discover and download custom scripts for kirka.io';
...
function buildOgHtml({ title, owner, description, image, slug }) {
  ...
  const ogUrl = escapeHtml(`${SITE_URL}/scripts/${slug}`);
  const canonical = escapeHtml(`${SITE_URL}/assets.html#scripts#${encodeURIComponent(slug)}`);
...
<head>
  ...
  <script>
    window.location.replace(${JSON.stringify(`${SITE_URL}/assets.html#scripts#${slug}`)});
  </script>
</head>
<body>
...
export async function onRequestGet(context) {
  ...
  if (!isBotRequest(request)) {
    return Response.redirect(`${SITE_URL}/assets.html#scripts#${encodeURIComponent(slug)}`, 302);
  }
  ...
  try {
    ...
    const items = (await res.json()).scripts || [];
    const targetName = slugToName(slug);
    const item = items.find((i) => (i.name || '').trim().toLowerCase() === targetName);

    if (!item) return Response.redirect(`${SITE_URL}/assets.html#scripts`, 302);

    return new Response(buildOgHtml({ title: item.name, owner: item.owner || 'Unknown', description: item.description || '', image: '', slug }), {
      status: 200, headers: { 'Content-Type': 'text/html;charset=UTF-8', 'Cache-Control': 'public, max-age=300, s-maxage=300' }
    });
  } catch (err) {
    return Response.redirect(`${SITE_URL}/assets.html#scripts`, 302);
  }
}
